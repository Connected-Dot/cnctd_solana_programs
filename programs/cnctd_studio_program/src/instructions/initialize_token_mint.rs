...
use anchor_spl::token_2022::{self, SetAuthority, Token2022, MintTo};
...
pub fn initialize_token_mint(ctx: Context<InitializeTokenMint>) -> Result<()> {   
    ...
    // Derive Treasury PDA's ATA for this mint - actually doing this off-chain but checking here
    // let treasury_ata = get_associated_token_address(&treasury_pda.key(), &token_mint.key());
    // msg!("Treasury ATA Derived: {:?}", treasury_ata);
    ...
    // let binding: &[&[&[u8]]] = &[&[signer.key.as_ref()]];
    ...
    token_2022::set_authority(cpi_ctx, AuthorityType::MintTokens, Some(treasury_pda.key()))?;
